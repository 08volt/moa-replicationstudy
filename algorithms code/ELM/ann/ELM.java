...
package moa.dabrze.classifiers.ann;
...
import moa.dabrze.linalg.Linalg;
...
public class ELM extends AbstractClassifier implements MultiClassClassifier
{
    ...
    public double[] getVotesForInstance(Instance instance) {
        ...
        INDArray H = getInstanceData(instance).mmul(alpha).add(bias);
        return Transforms.sigmoid(H).mmul(beta).data().asDouble();
    }
    ...
    public static INDArray getInstanceData(Instance instance)
    {
        ...
        float[][] inp = new float[1][input.size()];
        for(int i = 0; i<input.size();i++){
            inp[0][i] = input.get(i).floatValue();
        }
        return Nd4j.create(inp);
    }
